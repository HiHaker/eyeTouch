...
export default class IndexView extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            checkTable: "pimage", //选中的表名 用于modal 弹框标记
            flag: -1, //按钮状态
        }
    ...
    onChangeTab = (tabKey) => {
        if (tabKey !== "uploadFill") { // 判断是否文件上传
            const {pimageObj, pimageIndex, searchParam} = this.props;
            const {pageSize} = this.props[tabKey + "Obj"];
            const {id: search_NA} = pimageObj.list[pimageIndex] || {};
    ...
    onPrint = () => {
        const {pimageIndex, pimageObj} = this.props;
        const {list} = pimageObj;
        if (list.length === 0) {
            Warning('请选择需打印的数据');
            return;
        }
        const {id} = list[pimageIndex] || {};
    ...
    async confirmGoBack(type) {
        ...
        if (type === 1 && list.length > 0) {
            if (checkTable === "pimage") { // 主表
                const {pimageIndex} = this.props;
                const record = list[pimageIndex];
                await actions.masterDetailMany.delpimage(record);
            }
        }
    }
    /**
    ...
    render() {
        ...
        let selectRow = pimageObj['list'][pimageIndex] || {};
        // 主表数据为空
        const pimageForbid = pimageObj.list.length > 0 ? false : true;
        let {list} = pimageObj;
        const {id} = list[pimageIndex] || {};
        ...
        return (
            <div className='master-detail-many'>
                <Header title='帖子图片'/>
                <SearchArea pimageObj={pimageObj} onRef={this.onRef}/>
                <div className='table-header'>
                    <ButtonRoleGroup funcCode="pimage">
                        <Button
                            className="ml8"
                            role="add"
                            colors="primary"
                            onClick={() => this.onShowMainModal('pimage', 0)}
                        >新增</Button>
                        <Button
                            className="ml8"
                            role="update"
                            shape='border'
                            disabled={pimageForbid}
                            onClick={() => _this.onShowMainModal("pimage", 1)}
                        >修改</Button>
                        <Button
                            className="ml8"
                            shape='border'
                            disabled={pimageForbid}
                            onClick={() => _this.onShowMainModal("pimage", 2)}
                        >详情</Button>
                        <Button
                            className="ml8"
                            role="delete"
                            shape='border'
                            disabled={pimageForbid}
                            onClick={() => _this.onClickDel("pimage")}
                        >删除</Button>
                    </ButtonRoleGroup>
                </div>
                <Grid
                    ...
                    data={pimageObj.list}
                    rowKey={(r, i) => i}
                    columns={_this.pimageColumn}
                    getSelectedDataFunc={this.getSelectedDataFunc}
                    showHeaderMenu={true}
                    draggable={true}
                    multiSelect={false}
                    onRowClick={(record, index) => {
                        actions.masterDetailMany.updateState({pimageIndex: index});
                        // 根据tab 页来获取子表数据
                        const {
                            pimageObj, 
                            tabKey, 
                    ...
                    rowClassName={(record, index, indent) => { //判断是否选中当前行
                        return pimageIndex === index ? "selected" : "";
                    }}
                    paginationObj={{
                        ...this.getBasicPage(pimageObj),
                        freshData: (pageSize) => {
                            _this.freshData(pageSize, "pimageObj");
                        },
                        onDataNumSelect: (index, value) => {
                            _this.onDataNumSelect(index, value, "pimageObj");
                        },
                        dataNum: 0,
                    }}
                />
                <div className="table-space"> </div>
                <div className={pimageForbid? "tabel-header-wrap-hide":"tabel-header-wrap"} >
                    <Tabs
