...
export default class IndexView extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            checkTable: "pvideo", //选中的表名 用于modal 弹框标记
            flag: -1, //按钮状态
        }
    ...
    onChangeTab = (tabKey) => {
        if (tabKey !== "uploadFill") { // 判断是否文件上传
            const {pvideoObj, pvideoIndex, searchParam} = this.props;
            const {pageSize} = this.props[tabKey + "Obj"];
            const {id: search_NA} = pvideoObj.list[pvideoIndex] || {};
    ...
    onPrint = () => {
        const {pvideoIndex, pvideoObj} = this.props;
        const {list} = pvideoObj;
        if (list.length === 0) {
            Warning('请选择需打印的数据');
            return;
        }
        const {id} = list[pvideoIndex] || {};
    ...
    async confirmGoBack(type) {
        ...
        if (type === 1 && list.length > 0) {
            if (checkTable === "pvideo") { // 主表
                const {pvideoIndex} = this.props;
                const record = list[pvideoIndex];
                await actions.masterDetailMany.delpvideo(record);
            }
        }
    }
    /**
    ...
    render() {
        ...
        let selectRow = pvideoObj['list'][pvideoIndex] || {};
        // 主表数据为空
        const pvideoForbid = pvideoObj.list.length > 0 ? false : true;
        let {list} = pvideoObj;
        const {id} = list[pvideoIndex] || {};
        ...
        return (
            <div className='master-detail-many'>
                <Header title='帖子视频'/>
                <SearchArea pvideoObj={pvideoObj} onRef={this.onRef}/>
                <div className='table-header'>
                    <ButtonRoleGroup funcCode="pvideo">
                        <Button
                            ...
                            role="add"
                            colors="primary"
                            onClick={() => this.onShowMainModal('pvideo', 0)}
                        >新增</Button>
                        <Button
                            className="ml8"
                            role="update"
                            shape='border'
                            disabled={pvideoForbid}
                            onClick={() => _this.onShowMainModal("pvideo", 1)}
                        >修改</Button>
                        <Button
                            className="ml8"
                            shape='border'
                            disabled={pvideoForbid}
                            onClick={() => _this.onShowMainModal("pvideo", 2)}
                        >详情</Button>
                        <Button
                            className="ml8"
                            role="delete"
                            shape='border'
                            disabled={pvideoForbid}
                            onClick={() => _this.onClickDel("pvideo")}
                        >删除</Button>
                    </ButtonRoleGroup>
                </div>
                <Grid
                    ...
                    data={pvideoObj.list}
                    rowKey={(r, i) => i}
                    columns={_this.pvideoColumn}
                    getSelectedDataFunc={this.getSelectedDataFunc}
                    showHeaderMenu={true}
                    draggable={true}
                    multiSelect={false}
                    onRowClick={(record, index) => {
                        actions.masterDetailMany.updateState({pvideoIndex: index});
                        // 根据tab 页来获取子表数据
                        const {
                            pvideoObj, 
                            tabKey, 
                    ...
                    rowClassName={(record, index, indent) => { //判断是否选中当前行
                        return pvideoIndex === index ? "selected" : "";
                    }}
                    paginationObj={{
                        ...this.getBasicPage(pvideoObj),
                        freshData: (pageSize) => {
                            _this.freshData(pageSize, "pvideoObj");
                        },
                        onDataNumSelect: (index, value) => {
                            _this.onDataNumSelect(index, value, "pvideoObj");
                        },
                        dataNum: 0,
                    }}
                />
                <div className="table-space"> </div>
                <div className={pvideoForbid? "tabel-header-wrap-hide":"tabel-header-wrap"} >
                    <Tabs
