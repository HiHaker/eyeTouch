...
export default class IndexView extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            checkTable: "cbrand", //选中的表名 用于modal 弹框标记
            flag: -1, //按钮状态
        }
    ...
    onChangeTab = (tabKey) => {
        if (tabKey !== "uploadFill") { // 判断是否文件上传
            const {cbrandObj, cbrandIndex, searchParam} = this.props;
            const {pageSize} = this.props[tabKey + "Obj"];
            const {id: search_NA} = cbrandObj.list[cbrandIndex] || {};
    ...
    onPrint = () => {
        const {cbrandIndex, cbrandObj} = this.props;
        const {list} = cbrandObj;
        if (list.length === 0) {
            Warning('请选择需打印的数据');
            return;
        }
        const {id} = list[cbrandIndex] || {};
    ...
    async confirmGoBack(type) {
        ...
        if (type === 1 && list.length > 0) {
            if (checkTable === "cbrand") { // 主表
                const {cbrandIndex} = this.props;
                const record = list[cbrandIndex];
                await actions.masterDetailMany.delcbrand(record);
            }
        }
    }
    /**
    ...
    render() {
        ...
        let selectRow = cbrandObj['list'][cbrandIndex] || {};
        // 主表数据为空
        const cbrandForbid = cbrandObj.list.length > 0 ? false : true;
        let {list} = cbrandObj;
        const {id} = list[cbrandIndex] || {};
        ...
        return (
            <div className='master-detail-many'>
                <Header title='商品品牌'/>
                <SearchArea cbrandObj={cbrandObj} onRef={this.onRef}/>
                <div className='table-header'>
                    <ButtonRoleGroup funcCode="cbrand">
                        <Button
                            className="ml8"
                            role="add"
                            colors="primary"
                            onClick={() => this.onShowMainModal('cbrand', 0)}
                        >新增</Button>
                        <Button
                            className="ml8"
                            role="update"
                            shape='border'
                            disabled={cbrandForbid}
                            onClick={() => _this.onShowMainModal("cbrand", 1)}
                        >修改</Button>
                        <Button
                            className="ml8"
                            shape='border'
                            disabled={cbrandForbid}
                            onClick={() => _this.onShowMainModal("cbrand", 2)}
                        >详情</Button>
                        <Button
                            className="ml8"
                            role="delete"
                            shape='border'
                            disabled={cbrandForbid}
                            onClick={() => _this.onClickDel("cbrand")}
                        >删除</Button>
                    </ButtonRoleGroup>
                </div>
                <Grid
                    ...
                    data={cbrandObj.list}
                    rowKey={(r, i) => i}
                    columns={_this.cbrandColumn}
                    getSelectedDataFunc={this.getSelectedDataFunc}
                    showHeaderMenu={true}
                    draggable={true}
                    multiSelect={false}
                    onRowClick={(record, index) => {
                        actions.masterDetailMany.updateState({cbrandIndex: index});
                        // 根据tab 页来获取子表数据
                        const {
                            cbrandObj, 
                            tabKey, 
                    ...
                    rowClassName={(record, index, indent) => { //判断是否选中当前行
                        return cbrandIndex === index ? "selected" : "";
                    }}
                    paginationObj={{
                        ...this.getBasicPage(cbrandObj),
                        freshData: (pageSize) => {
                            _this.freshData(pageSize, "cbrandObj");
                        },
                        onDataNumSelect: (index, value) => {
                            _this.onDataNumSelect(index, value, "cbrandObj");
                        },
                        dataNum: 0,
                    }}
                />
                <div className="table-space"> </div>
                <div className={cbrandForbid? "tabel-header-wrap-hide":"tabel-header-wrap"} >
                    <Tabs
