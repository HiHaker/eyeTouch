...
class AddEditmyimg extends Component {
    constructor(props) {
        super(props);
        this.state = {
            rowData: {},
    ...
    componentDidMount(){
        ...
        const { myimgObj, myimgIndex: currentIndex } = this.props;
        // 防止网络阻塞造成btnFlag显示不正常
        this.setState({btnFlag: btnFlag});
        let rowData = {};
        try {
            // 判断是否重后端请求数据
            if (btnFlag > 0 && checkTable === "myimg") {
    ...
    onSubmitEdit = () => {
        ...
        this.props.form.validateFields(async (err, values) => {
            if (!err) {
                ...
                actions.masterDetailMany.savemyimg(values); //保存主表数据
                //返回
    ...
    render() {
        ...
        <FormControl disabled={typeof btnFlag != 'undefined' && btnFlag == 2
            ...
            ...getFieldProps('course', {
                ...
                initialValue: (typeof rowData === 'undefined' || typeof rowData.course === 'undefined') ? "" : rowData.course
,
                rules: [{
                    type:'string',required: false,pattern:/\S+/ig, message: '请输入教程',
                }],
                onChange(value) {
if(typeof rowData !== 'undefined'){
    let tempRow = Object.assign({},rowData,{ course: value });
    _this.setState({
        rowData:tempRow
    })
}
        ...
        />
        <FormError errorMsg={getFieldError('course')}/>
                                </FormItem>
                </FormList>
            </div>
        )
...
export default FormList.createForm()(AddEditmyimg);
