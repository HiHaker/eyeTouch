...
export default class IndexView extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            checkTable: "cimage", //选中的表名 用于modal 弹框标记
            flag: -1, //按钮状态
        }
    ...
    onChangeTab = (tabKey) => {
        if (tabKey !== "uploadFill") { // 判断是否文件上传
            const {cimageObj, cimageIndex, searchParam} = this.props;
            const {pageSize} = this.props[tabKey + "Obj"];
            const {id: search_NA} = cimageObj.list[cimageIndex] || {};
    ...
    onPrint = () => {
        const {cimageIndex, cimageObj} = this.props;
        const {list} = cimageObj;
        if (list.length === 0) {
            Warning('请选择需打印的数据');
            return;
        }
        const {id} = list[cimageIndex] || {};
    ...
    async confirmGoBack(type) {
        ...
        if (type === 1 && list.length > 0) {
            if (checkTable === "cimage") { // 主表
                const {cimageIndex} = this.props;
                const record = list[cimageIndex];
                await actions.masterDetailMany.delcimage(record);
            }
        }
    }
    /**
    ...
    render() {
        ...
        let selectRow = cimageObj['list'][cimageIndex] || {};
        // 主表数据为空
        const cimageForbid = cimageObj.list.length > 0 ? false : true;
        let {list} = cimageObj;
        const {id} = list[cimageIndex] || {};
        ...
        return (
            <div className='master-detail-many'>
                <Header title='商品图片'/>
                <SearchArea cimageObj={cimageObj} onRef={this.onRef}/>
                <div className='table-header'>
                    <ButtonRoleGroup funcCode="cimage">
                        <Button
                            className="ml8"
                            role="add"
                            colors="primary"
                            onClick={() => this.onShowMainModal('cimage', 0)}
                        >新增</Button>
                        <Button
                            className="ml8"
                            role="update"
                            shape='border'
                            disabled={cimageForbid}
                            onClick={() => _this.onShowMainModal("cimage", 1)}
                        >修改</Button>
                        <Button
                            className="ml8"
                            shape='border'
                            disabled={cimageForbid}
                            onClick={() => _this.onShowMainModal("cimage", 2)}
                        >详情</Button>
                        <Button
                            className="ml8"
                            role="delete"
                            shape='border'
                            disabled={cimageForbid}
                            onClick={() => _this.onClickDel("cimage")}
                        >删除</Button>
                    </ButtonRoleGroup>
                </div>
                <Grid
                    ...
                    data={cimageObj.list}
                    rowKey={(r, i) => i}
                    columns={_this.cimageColumn}
                    getSelectedDataFunc={this.getSelectedDataFunc}
                    showHeaderMenu={true}
                    draggable={true}
                    multiSelect={false}
                    onRowClick={(record, index) => {
                        actions.masterDetailMany.updateState({cimageIndex: index});
                        // 根据tab 页来获取子表数据
                        const {
                            cimageObj, 
                            tabKey, 
                    ...
                    rowClassName={(record, index, indent) => { //判断是否选中当前行
                        return cimageIndex === index ? "selected" : "";
                    }}
                    paginationObj={{
                        ...this.getBasicPage(cimageObj),
                        freshData: (pageSize) => {
                            _this.freshData(pageSize, "cimageObj");
                        },
                        onDataNumSelect: (index, value) => {
                            _this.onDataNumSelect(index, value, "cimageObj");
                        },
                        dataNum: 0,
                    }}
                />
                <div className="table-space"> </div>
                <div className={cimageForbid? "tabel-header-wrap-hide":"tabel-header-wrap"} >
                    <Tabs
