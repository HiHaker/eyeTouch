...
export default class IndexView extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            checkTable: "myuser", //选中的表名 用于modal 弹框标记
            flag: -1, //按钮状态
        }
    ...
    onChangeTab = (tabKey) => {
        if (tabKey !== "uploadFill") { // 判断是否文件上传
            const {myuserObj, myuserIndex, searchParam} = this.props;
            const {pageSize} = this.props[tabKey + "Obj"];
            const {id: search_NA} = myuserObj.list[myuserIndex] || {};
    ...
    onPrint = () => {
        const {myuserIndex, myuserObj} = this.props;
        const {list} = myuserObj;
        if (list.length === 0) {
            Warning('请选择需打印的数据');
            return;
        }
        const {id} = list[myuserIndex] || {};
    ...
    async confirmGoBack(type) {
        ...
        if (type === 1 && list.length > 0) {
            if (checkTable === "myuser") { // 主表
                const {myuserIndex} = this.props;
                const record = list[myuserIndex];
                await actions.masterDetailMany.delmyuser(record);
            }
        }
    }
    /**
    ...
    render() {
        ...
        let selectRow = myuserObj['list'][myuserIndex] || {};
        // 主表数据为空
        const myuserForbid = myuserObj.list.length > 0 ? false : true;
        let {list} = myuserObj;
        const {id} = list[myuserIndex] || {};
        ...
        return (
            <div className='master-detail-many'>
                <Header title='平台用户'/>
                <SearchArea myuserObj={myuserObj} onRef={this.onRef}/>
                <div className='table-header'>
                    <ButtonRoleGroup funcCode="myuser">
                        <Button
                            className="ml8"
                            role="add"
                            colors="primary"
                            onClick={() => this.onShowMainModal('myuser', 0)}
                        >新增</Button>
                        <Button
                            className="ml8"
                            role="update"
                            shape='border'
                            disabled={myuserForbid}
                            onClick={() => _this.onShowMainModal("myuser", 1)}
                        >修改</Button>
                        <Button
                            className="ml8"
                            shape='border'
                            disabled={myuserForbid}
                            onClick={() => _this.onShowMainModal("myuser", 2)}
                        >详情</Button>
                        <Button
                            className="ml8"
                            role="delete"
                            shape='border'
                            disabled={myuserForbid}
                            onClick={() => _this.onClickDel("myuser")}
                        >删除</Button>
                    </ButtonRoleGroup>
                </div>
                <Grid
                    ...
                    data={myuserObj.list}
                    rowKey={(r, i) => i}
                    columns={_this.myuserColumn}
                    getSelectedDataFunc={this.getSelectedDataFunc}
                    showHeaderMenu={true}
                    draggable={true}
                    multiSelect={false}
                    onRowClick={(record, index) => {
                        actions.masterDetailMany.updateState({myuserIndex: index});
                        // 根据tab 页来获取子表数据
                        const {
                            myuserObj, 
                            tabKey, 
                    ...
                    rowClassName={(record, index, indent) => { //判断是否选中当前行
                        return myuserIndex === index ? "selected" : "";
                    }}
                    paginationObj={{
                        ...this.getBasicPage(myuserObj),
                        freshData: (pageSize) => {
                            _this.freshData(pageSize, "myuserObj");
                        },
                        onDataNumSelect: (index, value) => {
                            _this.onDataNumSelect(index, value, "myuserObj");
                        },
                        dataNum: 0,
                    }}
                />
                <div className="table-space"> </div>
                <div className={myuserForbid? "tabel-header-wrap-hide":"tabel-header-wrap"} >
                    <Tabs
