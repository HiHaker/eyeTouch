...
class AddEditmyuser extends Component {
    constructor(props) {
        super(props);
        this.state = {
            rowData: {},
    ...
    componentDidMount(){
        ...
        const { myuserObj, myuserIndex: currentIndex } = this.props;
        // 防止网络阻塞造成btnFlag显示不正常
        this.setState({btnFlag: btnFlag});
        let rowData = {};
        try {
            // 判断是否重后端请求数据
            if (btnFlag > 0 && checkTable === "myuser") {
    ...
    onSubmitEdit = () => {
        ...
        this.props.form.validateFields(async (err, values) => {
            if (!err) {
                ...
                actions.masterDetailMany.savemyuser(values); //保存主表数据
                //返回
...
,
                rules: [{
                    type:'string',required: false,pattern:/\S+/ig, message: '请输入昵称',
                }],
                onChange(value) {
if(typeof rowData !== 'undefined'){
...
export default FormList.createForm()(AddEditmyuser);
