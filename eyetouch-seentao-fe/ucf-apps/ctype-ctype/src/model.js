...
export default {
    ...
    effects: {
        ...
        async loadList(param, getState) {
            ...
            const {result} = processData(await api.getctype(param));  // 调用 getList 请求数据
            const {data: resctype} = result;
            const {content = []} = resctype || {};

            if (content.length > 0) { // 获取子表数据
                const ctypeObj = structureObj(resctype, param);
                actions.masterDetailMany.updateState({ctypeObj}); // 更新主表数据
            } else {
                 const {
                    ctypeObj,
                } = getState().masterDetailMany;
                actions.masterDetailMany.updateState({   // 如果请求出错,数据初始化
                        ctypeObj: initStateObj(ctypeObj),
                        ctypeRow: {}
                    }
                );
            }
            actions.masterDetailMany.updateState({showLoading: false, ctypeIndex: 0});
        },
        /**
         * getSelect：保存主表数据
         * @param {*} param
         * @param {*} getState
         */
        async savectype(param, getState) {
            actions.masterDetailMany.updateState({showLoading: true});   // 正在加载数据，显示加载 Loading 图标
            const {btnFlag} = param;
            let status = null;
            delete param.btnFlag; //删除标识字段
            if (btnFlag === 0) { // 添加数据
                const {result} = processData(await api.savectype(param), '添加成功');
                status = result.status;
            }
            if (btnFlag === 1) { // 修改数据
                const {result} = processData(await api.updatectype(param), '修改成功');
                status = result.status;
            }

            if (status === 'success') { // 如果不判断是会报错，param参数有错
                const {pageSize} = getState().masterDetailMany.ctypeObj;
                // 带上子表信息
         ...
         * @param {*} getState
         */
        async delctype(param, getState) {
            const {id} = param;
            const {result}=processData(await api.delctype([{id}]), '删除成功');
            const {status}=result;
            if(status==='success'){
                // 获取表pageSize;
                const {ctypeObj} = getState().masterDetailMany;
                const {pageSize} = ctypeObj;
         ...
         * @param {*} getState
         */
        async queryctype(param, getState) {
