...
class AddEditctype extends Component {
    constructor(props) {
        super(props);
        this.state = {
            rowData: {},
    ...
    componentDidMount(){
        ...
        const { ctypeObj, ctypeIndex: currentIndex } = this.props;
        // 防止网络阻塞造成btnFlag显示不正常
        this.setState({btnFlag: btnFlag});
        let rowData = {};
        try {
            // 判断是否重后端请求数据
            if (btnFlag > 0 && checkTable === "ctype") {
    ...
    onSubmitEdit = () => {
        ...
        this.props.form.validateFields(async (err, values) => {
            if (!err) {
                ...
                actions.masterDetailMany.savectype(values); //保存主表数据
                //返回
...
,
                rules: [{
                    type:'string',required: false,pattern:/\S+/ig, message: '请输入商品类型名称',
                }],
                onChange(value) {
...
export default FormList.createForm()(AddEditctype);
