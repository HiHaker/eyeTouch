...
export default class IndexView extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            checkTable: "pfavorites", //选中的表名 用于modal 弹框标记
            flag: -1, //按钮状态
        }
    ...
    onChangeTab = (tabKey) => {
        if (tabKey !== "uploadFill") { // 判断是否文件上传
            const {pfavoritesObj, pfavoritesIndex, searchParam} = this.props;
            const {pageSize} = this.props[tabKey + "Obj"];
            const {id: search_NA} = pfavoritesObj.list[pfavoritesIndex] || {};
    ...
    onPrint = () => {
        const {pfavoritesIndex, pfavoritesObj} = this.props;
        const {list} = pfavoritesObj;
        if (list.length === 0) {
            Warning('请选择需打印的数据');
            return;
        }
        const {id} = list[pfavoritesIndex] || {};
    ...
    async confirmGoBack(type) {
        ...
        if (type === 1 && list.length > 0) {
            if (checkTable === "pfavorites") { // 主表
                const {pfavoritesIndex} = this.props;
                const record = list[pfavoritesIndex];
                await actions.masterDetailMany.delpfavorites(record);
            }
        }
    }
    /**
    ...
    render() {
        ...
        let selectRow = pfavoritesObj['list'][pfavoritesIndex] || {};
        // 主表数据为空
        const pfavoritesForbid = pfavoritesObj.list.length > 0 ? false : true;
        let {list} = pfavoritesObj;
        const {id} = list[pfavoritesIndex] || {};

        return (
            <div className='master-detail-many'>
                <Header title='帖子收藏'/>
                <SearchArea pfavoritesObj={pfavoritesObj} onRef={this.onRef}/>
                <div className='table-header'>
                    <ButtonRoleGroup funcCode="pfavorites">
                        <Button
                            className="ml8"
                            role="add"
                            colors="primary"
                            onClick={() => this.onShowMainModal('pfavorites', 0)}
                        >新增</Button>
                        <Button
                            className="ml8"
                            role="update"
                            shape='border'
                            disabled={pfavoritesForbid}
                            onClick={() => _this.onShowMainModal("pfavorites", 1)}
                        >修改</Button>
                        <Button
                            className="ml8"
                            shape='border'
                            disabled={pfavoritesForbid}
                            onClick={() => _this.onShowMainModal("pfavorites", 2)}
                        >详情</Button>
                        <Button
                            className="ml8"
                            role="delete"
                            shape='border'
                            disabled={pfavoritesForbid}
                            onClick={() => _this.onClickDel("pfavorites")}
                        >删除</Button>
                    </ButtonRoleGroup>
                </div>
                <Grid
                    ref="pfavorites"
                    data={pfavoritesObj.list}
                    rowKey={(r, i) => i}
                    columns={_this.pfavoritesColumn}
                    getSelectedDataFunc={this.getSelectedDataFunc}
                    showHeaderMenu={true}
                    draggable={true}
                    multiSelect={false}
                    onRowClick={(record, index) => {
                        actions.masterDetailMany.updateState({pfavoritesIndex: index});
                        // 根据tab 页来获取子表数据
                        const {
                            pfavoritesObj, 
                            tabKey, 
                    ...
                    rowClassName={(record, index, indent) => { //判断是否选中当前行
                        return pfavoritesIndex === index ? "selected" : "";
                    }}
                    paginationObj={{
                        ...this.getBasicPage(pfavoritesObj),
                        freshData: (pageSize) => {
                            _this.freshData(pageSize, "pfavoritesObj");
                        },
                        onDataNumSelect: (index, value) => {
                            _this.onDataNumSelect(index, value, "pfavoritesObj");
                        },
                        dataNum: 0,
                    }}
                />
                <div className="table-space"> </div>
                <div className={pfavoritesForbid? "tabel-header-wrap-hide":"tabel-header-wrap"} >
                    <Tabs
