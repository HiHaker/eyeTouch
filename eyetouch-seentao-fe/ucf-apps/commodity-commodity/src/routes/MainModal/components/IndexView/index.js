...
class AddEditcommodity extends Component {
    constructor(props) {
        super(props);
        this.state = {
            rowData: {},
    ...
    componentDidMount(){
        ...
        const { commodityObj, commodityIndex: currentIndex } = this.props;
        // 防止网络阻塞造成btnFlag显示不正常
        this.setState({btnFlag: btnFlag});
        let rowData = {};
        try {
            // 判断是否重后端请求数据
            if (btnFlag > 0 && checkTable === "commodity") {
    ...
    onSubmitEdit = () => {
        ...
        this.props.form.validateFields(async (err, values) => {
            if (!err) {
                ...
                actions.masterDetailMany.savecommodity(values); //保存主表数据
                //返回
...
if(typeof rowData !== 'undefined'){
    ...
    _this.setState({
        ...
        />
        <FormError errorMsg={getFieldError('content')}/>
                                </FormItem>
                                <FormItem>
        <Label >
            商品功效
...
if(typeof rowData !== 'undefined'){
    ...
    _this.setState({
        ...
        />
        <FormError errorMsg={getFieldError('effacicy')}/>
                                </FormItem>
                                <FormItem>
        <Label >
            商品名称
...
if(typeof rowData !== 'undefined'){
    ...
    _this.setState({
        ...
        />
        <FormError errorMsg={getFieldError('link')}/>
                                </FormItem>
                </FormList>
            </div>
        )
...
export default FormList.createForm()(AddEditcommodity);
