...
class AddEditrelation extends Component {
    constructor(props) {
        super(props);
        this.state = {
            rowData: {},
    ...
    componentDidMount(){
        ...
        const { relationObj, relationIndex: currentIndex } = this.props;
        // 防止网络阻塞造成btnFlag显示不正常
        this.setState({btnFlag: btnFlag});
        let rowData = {};
        try {
            // 判断是否重后端请求数据
            if (btnFlag > 0 && checkTable === "relation") {
    ...
    onSubmitEdit = () => {
        ...
        this.props.form.validateFields(async (err, values) => {
            if (!err) {
                ...
                actions.masterDetailMany.saverelation(values); //保存主表数据
                //返回
...
if(typeof rowData !== 'undefined'){
    ...
    _this.setState({
        ...
        />
        <FormError errorMsg={getFieldError('follows')}/>
                                </FormItem>
                                <FormItem>
        <Label >
            粉丝id
...
if(typeof rowData !== 'undefined'){
    ...
    _this.setState({
        ...
        />
        <FormError errorMsg={getFieldError('fans')}/>
                                </FormItem>
                </FormList>
            </div>
        )
...
export default FormList.createForm()(AddEditrelation);
