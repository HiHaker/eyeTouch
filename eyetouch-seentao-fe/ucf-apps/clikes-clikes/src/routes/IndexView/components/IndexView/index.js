...
export default class IndexView extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            checkTable: "clikes", //选中的表名 用于modal 弹框标记
            flag: -1, //按钮状态
        }
    ...
    onChangeTab = (tabKey) => {
        if (tabKey !== "uploadFill") { // 判断是否文件上传
            const {clikesObj, clikesIndex, searchParam} = this.props;
            const {pageSize} = this.props[tabKey + "Obj"];
            const {id: search_NA} = clikesObj.list[clikesIndex] || {};
    ...
    onPrint = () => {
        const {clikesIndex, clikesObj} = this.props;
        const {list} = clikesObj;
        if (list.length === 0) {
            Warning('请选择需打印的数据');
            return;
        }
        const {id} = list[clikesIndex] || {};
    ...
    async confirmGoBack(type) {
        ...
        if (type === 1 && list.length > 0) {
            if (checkTable === "clikes") { // 主表
                const {clikesIndex} = this.props;
                const record = list[clikesIndex];
                await actions.masterDetailMany.delclikes(record);
            }
        }
    }
    /**
    ...
    render() {
        ...
        let selectRow = clikesObj['list'][clikesIndex] || {};
        // 主表数据为空
        const clikesForbid = clikesObj.list.length > 0 ? false : true;
        let {list} = clikesObj;
        const {id} = list[clikesIndex] || {};
        ...
        return (
            <div className='master-detail-many'>
                <Header title='商品点赞'/>
                <SearchArea clikesObj={clikesObj} onRef={this.onRef}/>
                <div className='table-header'>
                    <ButtonRoleGroup funcCode="clikes">
                        <Button
                            className="ml8"
                            role="add"
                            colors="primary"
                            onClick={() => this.onShowMainModal('clikes', 0)}
                        >新增</Button>
                        <Button
                            className="ml8"
                            role="update"
                            shape='border'
                            disabled={clikesForbid}
                            onClick={() => _this.onShowMainModal("clikes", 1)}
                        >修改</Button>
                        <Button
                            className="ml8"
                            shape='border'
                            disabled={clikesForbid}
                            onClick={() => _this.onShowMainModal("clikes", 2)}
                        >详情</Button>
                        <Button
                            className="ml8"
                            role="delete"
                            shape='border'
                            disabled={clikesForbid}
                            onClick={() => _this.onClickDel("clikes")}
                        >删除</Button>
                    </ButtonRoleGroup>
                </div>
                <Grid
                    ...
                    data={clikesObj.list}
                    rowKey={(r, i) => i}
                    columns={_this.clikesColumn}
                    getSelectedDataFunc={this.getSelectedDataFunc}
                    showHeaderMenu={true}
                    draggable={true}
                    multiSelect={false}
                    onRowClick={(record, index) => {
                        actions.masterDetailMany.updateState({clikesIndex: index});
                        // 根据tab 页来获取子表数据
                        const {
                            clikesObj, 
                            tabKey, 
                    ...
                    rowClassName={(record, index, indent) => { //判断是否选中当前行
                        return clikesIndex === index ? "selected" : "";
                    }}
                    paginationObj={{
                        ...this.getBasicPage(clikesObj),
                        freshData: (pageSize) => {
                            _this.freshData(pageSize, "clikesObj");
                        },
                        onDataNumSelect: (index, value) => {
                            _this.onDataNumSelect(index, value, "clikesObj");
                        },
                        dataNum: 0,
                    }}
                />
                <div className="table-space"> </div>
                <div className={clikesForbid? "tabel-header-wrap-hide":"tabel-header-wrap"} >
                    <Tabs
