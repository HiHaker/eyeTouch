...
class AddEditeffacicy extends Component {
    constructor(props) {
        super(props);
        this.state = {
            rowData: {},
    ...
    componentDidMount(){
        ...
        const { effacicyObj, effacicyIndex: currentIndex } = this.props;
        // 防止网络阻塞造成btnFlag显示不正常
        this.setState({btnFlag: btnFlag});
        let rowData = {};
        try {
            // 判断是否重后端请求数据
            if (btnFlag > 0 && checkTable === "effacicy") {
    ...
    onSubmitEdit = () => {
        ...
        this.props.form.validateFields(async (err, values) => {
            if (!err) {
                ...
                actions.masterDetailMany.saveeffacicy(values); //保存主表数据
                //返回
...
,
                rules: [{
                    type:'string',required: false,pattern:/\S+/ig, message: '请输入商品功效名称',
                }],
                onChange(value) {
...
export default FormList.createForm()(AddEditeffacicy);
