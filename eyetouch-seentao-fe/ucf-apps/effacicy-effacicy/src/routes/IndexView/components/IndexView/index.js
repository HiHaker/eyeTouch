...
export default class IndexView extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            checkTable: "effacicy", //选中的表名 用于modal 弹框标记
            flag: -1, //按钮状态
        }
    ...
    onChangeTab = (tabKey) => {
        if (tabKey !== "uploadFill") { // 判断是否文件上传
            const {effacicyObj, effacicyIndex, searchParam} = this.props;
            const {pageSize} = this.props[tabKey + "Obj"];
            const {id: search_NA} = effacicyObj.list[effacicyIndex] || {};
    ...
    onPrint = () => {
        const {effacicyIndex, effacicyObj} = this.props;
        const {list} = effacicyObj;
        if (list.length === 0) {
            Warning('请选择需打印的数据');
            return;
        }
        const {id} = list[effacicyIndex] || {};
    ...
    async confirmGoBack(type) {
        ...
        if (type === 1 && list.length > 0) {
            if (checkTable === "effacicy") { // 主表
                const {effacicyIndex} = this.props;
                const record = list[effacicyIndex];
                await actions.masterDetailMany.deleffacicy(record);
            }
        }
    }
    /**
    ...
    render() {
        ...
        let selectRow = effacicyObj['list'][effacicyIndex] || {};
        // 主表数据为空
        const effacicyForbid = effacicyObj.list.length > 0 ? false : true;
        let {list} = effacicyObj;
        const {id} = list[effacicyIndex] || {};

        return (
            <div className='master-detail-many'>
                <Header title='商品功效'/>
                <SearchArea effacicyObj={effacicyObj} onRef={this.onRef}/>
                <div className='table-header'>
                    <ButtonRoleGroup funcCode="effacicy">
                        <Button
                            className="ml8"
                            role="add"
                            colors="primary"
                            onClick={() => this.onShowMainModal('effacicy', 0)}
                        >新增</Button>
                        <Button
                            className="ml8"
                            role="update"
                            shape='border'
                            disabled={effacicyForbid}
                            onClick={() => _this.onShowMainModal("effacicy", 1)}
                        >修改</Button>
                        <Button
                            className="ml8"
                            shape='border'
                            disabled={effacicyForbid}
                            onClick={() => _this.onShowMainModal("effacicy", 2)}
                        >详情</Button>
                        <Button
                            className="ml8"
                            role="delete"
                            shape='border'
                            disabled={effacicyForbid}
                            onClick={() => _this.onClickDel("effacicy")}
                        >删除</Button>
                    </ButtonRoleGroup>
                </div>
                <Grid
                    ref="effacicy"
                    data={effacicyObj.list}
                    rowKey={(r, i) => i}
                    columns={_this.effacicyColumn}
                    getSelectedDataFunc={this.getSelectedDataFunc}
                    showHeaderMenu={true}
                    draggable={true}
                    multiSelect={false}
                    onRowClick={(record, index) => {
                        actions.masterDetailMany.updateState({effacicyIndex: index});
                        // 根据tab 页来获取子表数据
                        const {
                            effacicyObj, 
                            tabKey, 
                    ...
                    rowClassName={(record, index, indent) => { //判断是否选中当前行
                        return effacicyIndex === index ? "selected" : "";
                    }}
                    paginationObj={{
                        ...this.getBasicPage(effacicyObj),
                        freshData: (pageSize) => {
                            _this.freshData(pageSize, "effacicyObj");
                        },
                        onDataNumSelect: (index, value) => {
                            _this.onDataNumSelect(index, value, "effacicyObj");
                        },
                        dataNum: 0,
                    }}
                />
                <div className="table-space"> </div>
                <div className={effacicyForbid? "tabel-header-wrap-hide":"tabel-header-wrap"} >
                    <Tabs
