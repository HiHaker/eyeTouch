...
class AddEditpost extends Component {
    ...
    componentDidMount(){
        ...
        const { postObj, postIndex: currentIndex } = this.props;
        // 防止网络阻塞造成btnFlag显示不正常
        this.setState({btnFlag: btnFlag});
        let rowData = {};
        try {
            // 判断是否重后端请求数据
            if (btnFlag > 0 && checkTable === "post") {
    ...
    onSubmitEdit = () => {
        ...
        this.props.form.validateFields(async (err, values) => {
            if (!err) {
                ...
                actions.masterDetailMany.savepost(values); //保存主表数据
                //返回
...
if(typeof rowData !== 'undefined'){
    ...
    _this.setState({
        ...
        />
        <FormError errorMsg={getFieldError('style')}/>
                                </FormItem>
                                <FormItem>
        <Label >
            用户id
...
if(typeof rowData !== 'undefined'){
    ...
    _this.setState({
        ...
        />
        <FormError errorMsg={getFieldError('fpid')}/>
                                </FormItem>
                </FormList>
            </div>
        )
...
export default FormList.createForm()(AddEditpost);
