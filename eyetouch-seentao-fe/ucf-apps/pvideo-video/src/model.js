...
export default {
    ...
    effects: {
        ...
        async loadList(param, getState) {
            ...
            const {result} = processData(await api.getvideo(param));  // 调用 getList 请求数据
            const {data: resvideo} = result;
            const {content = []} = resvideo || {};

            if (content.length > 0) { // 获取子表数据
                const videoObj = structureObj(resvideo, param);
                actions.masterDetailMany.updateState({videoObj}); // 更新主表数据
            } else {
                 const {
                    videoObj,
                } = getState().masterDetailMany;
                actions.masterDetailMany.updateState({   // 如果请求出错,数据初始化
                        videoObj: initStateObj(videoObj),
                        videoRow: {}
                    }
                );
            }
            actions.masterDetailMany.updateState({showLoading: false, videoIndex: 0});
        },
        /**
         * getSelect：保存主表数据
         * @param {*} param
         * @param {*} getState
         */
        async savevideo(param, getState) {
            actions.masterDetailMany.updateState({showLoading: true});   // 正在加载数据，显示加载 Loading 图标
            const {btnFlag} = param;
            let status = null;
            delete param.btnFlag; //删除标识字段
            if (btnFlag === 0) { // 添加数据
                const {result} = processData(await api.savevideo(param), '添加成功');
                status = result.status;
            }
            if (btnFlag === 1) { // 修改数据
                const {result} = processData(await api.updatevideo(param), '修改成功');
                status = result.status;
            }

            if (status === 'success') { // 如果不判断是会报错，param参数有错
                const {pageSize} = getState().masterDetailMany.videoObj;
                // 带上子表信息
         ...
         * @param {*} getState
         */
        async delvideo(param, getState) {
            const {id} = param;
            const {result}=processData(await api.delvideo([{id}]), '删除成功');
            const {status}=result;
            if(status==='success'){
                // 获取表pageSize;
                const {videoObj} = getState().masterDetailMany;
                const {pageSize} = videoObj;
         ...
         * @param {*} getState
         */
        async queryvideo(param, getState) {
